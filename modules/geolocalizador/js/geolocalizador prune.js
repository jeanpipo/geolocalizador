...
function processesLocationService(res,p_posicion,radius,objFraccion){
	...
	res.forEach(function(value){
			...
			arr.push(ejemplo([value.lat,value.lng],"green",radius*10,value.informacion,objFraccion));
	});
	return arr;
}
...
function createPin(latlng,color,radius,text,posicionFraccion){
	...
	return (text) ? new  L.semiCircleMarker(latlng, {
    color: color,
    fillColor: color,
    fillOpacity: 0.3,
    radius: radius*2,
...
function ejemplo(position,color,radius,text,posicionFraccion){
...
// Esto es porque nosotros dibujamos los iconos individuales de los equipos por CSS pero se puede usar el icono default
/*
cluster.PrepareLeafletMarker = function(marker, data) {
    marker.setIcon(data.icono);
    marker.bindLabel(data.name);
};
*/

cluster.BuildLeafletCluster = function(cluster, position,color,radius,text,posicionFraccion) {

    var pruneCluster = cluster;

    var m = //createPin(position,color,radius,text,posicionFraccion);
		new L.Marker(position, {num: "special"/*,icon: cluster.BuildLeafletClusterIcon(cluster)*/});//.bindPopup(text);// */

		///*** esto estaba donde estaba createPin
    ...
    for (var i = 0; i < estados.length; i++) {
        if (estados[i] > 0) {
            popup[x] = estados[i] + ' ' + scannet.datos.severidades[i].nombre;
            x++
        }
    }

  //  m.bindPopup(popup.join("<br>"), { offset: new L.Point(0, -15) })
		cluster.Cluster.Size = 10;

  /*  m.on('click', function(t) {
        // Compute the  cluster bounds (it's slow : O(n))
				//alert(" jena  "+cluster.bounds);
				console.log(t.target._map.getBounds()._northEast);
        var markersArea = pruneCluster.Cluster.FindMarkersInArea(t.target._map.getBounds()._northEast);
				alert("pipos ");
    ...
    var equipos = [];
    for (var i = 0; i < cluster.population; i++) {
        equipos[i] = cluster._clusterMarkers[i].data.name;
    }
...
cluster.BuildLeafletClusterIcon = function(cluster) {
    ...
    for (var i = 0; i < cluster.population; i++) {
        var estado = parseInt(cluster._clusterMarkers[i].data.estado);
    ...
    var mayor_alarmado = Math.max.apply(Math, estados);
    e.mayor_alarmado = mayor_alarmado;
    e.total_alarmados = total_alarmados;
...
L.Icon.MarkerCluster = L.Icon.extend({
    ...
    draw: function(canvas, width, height) {
        ...
        var start = 0;
				alert("sdf");
        for (var i = 0, l = scannet.datos.severidades.length; i < l; ++i) {
					//alert(this.stats);
            ...
            if (size > 0) {
                ...
                canvas.fillStyle = scannet.datos.severidades[i].style.split(':')[1].trim();
                var from = start + 0.14,
                    to = start + size * pi2;
                if (to < from) {
                    from = start;
        ...
        canvas.fillStyle = severidades[this.mayor_alarmado].style.split(':')[1].trim();
        ...
        canvas.fillText(this.total_alarmados + '/' + this.population, 22, 22, 40);
    }
});
//cluster.stats;
//console.log(cluster);
...
cluster.RegisterMarker(cluster.BuildLeafletCluster(cluster, position,color,radius,text,posicionFraccion));


	map.addLayer(cluster.Cluster._markers[0]);
	//console.log(cluster.stats);
	return cluster.BuildLeafletCluster(cluster, position,color,radius,text,posicionFraccion);

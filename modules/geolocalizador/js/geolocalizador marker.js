...
var min =max-60;
var bufferOptions =[];
var circlos = new L.MarkerClusterGroup();
var infPop = {"infPop":[]};
...
function createPin(latlng,color,radius,text,posicionFraccion){
	...
	return (text) ?L.semiCircleMarker(latlng, {
    color: color,
    fillColor: color,
    fillOpacity: 0.3,
    radius: radius*1,
...
function addPin(pins){
	...
	//console.log(markers._showCoverage(markers.getLayers()[1]));
	//markers.getLayers()[2]
	//markers.getVisibleParent();
	map.addLayer(markers);
}

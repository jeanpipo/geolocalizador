		...
		while(keys[x]){
			...
			if(marker[keys[x]]==false){
				...
				//	console.log(x+" ff "+fracciones[bufferOptions.length-1][keys[bufferOptions.length-1]][x].startAngle);
					...
					//var thisIcon = new L.iconUrl("C:\Users\jsosa\Desktop\tortas\1.png");
...
function createPin(latlng,color,radius,text,posicionFraccion){
	...
	//for (var i = 0; i < keys.length; i++) {
	 ...
	 let mrkr = new PruneCluster.Marker(parseFloat(latlng[0]),parseFloat(latlng[1]),{popup:text});
	 mrkr.category=Math.floor(Math.random() * Math.random() * colors.length);
	 mrkr.data.optionName="pussy";
	 return mrkr;
...
function setEventMouseCluster(){
	...
	//console.log(cluster._objectsOnMap[0].data);
		cluster._objectsOnMap.forEach(value=>{
			console.log(cluster.Cluster.FindMarkersInArea(value.bounds));
			value.data._leafletMarker.bindPopup("heeey")
...
cluster.BuildLeafletClusterIcon = function(cluster) {
        ...
        e.population = cluster.population;
		    ...
		    var x = 0;
				...
				/*setTimeout(function () {
					console.log(cluster._clusterMarkers);
				}, 4000);*/
			/*	setTimeout(function () {
